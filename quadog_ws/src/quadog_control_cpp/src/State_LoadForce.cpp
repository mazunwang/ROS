...
void State_LoadForce::onEnter(){
    ...
    std::cout<< "-----------" << this->_legController->legName[curLegNum] <<" Leg LoadForce State Start"<<"----------"<<std::endl;
    cout<<x_pos_Init<<" "<<y_pos_Init<<" "<<h_pos_Init<<endl;
    // cout<<x_set<<" "<<y_set<<endl;
}

void State_LoadForce::run(){
    h_pos = 0.0;
    h_vel = 0.0;
    ...
    this->swingTraj->CubicPolynomialInterpolation(x_pos_Init,x_vel_Init,x_set,0,_time,_quaCP->LOAD_PERIOD,x_des_pos,x_des_vel);
    this->swingTraj->CubicPolynomialInterpolation(y_pos_Init,y_vel_Init,y_set,0,_time,_quaCP->LOAD_PERIOD,y_des_pos,y_des_vel);
    this->swingTraj->CubicPolynomialInterpolation(h_pos_Init,h_vel_Init,h_set,0,_time,_quaCP->LOAD_PERIOD,h_des_pos,h_des_vel);    
    ...
    this->swingTraj->LinerInterPolation(0,alpha,_time,_quaCP->LOAD_PERIOD,alpha_des,aplah_vel);
...
void State_LoadForce::onExit(){
    legNum++;
}

State_Name State_LoadForce::checkTransition(){
    if(_time>_quaCP->LOAD_PERIOD ) return State_Name::COM_MOVE;
    return State_Name::LOADFORCE;
}

TransitionData State_LoadForce::getTransitionData(){
    return this->_transitonData;
}


void State_LoadForce::getXYInitState(){
...
void State_LoadForce::getXYCurState(){
